...
model_dirs = [
    ...
    'model-Llama-2-7b-chat-hf-multi',
    'model-Llama-2-7b-hf',
]
...
q_val = [2,7,13,17,26,34,40,46,52,58,66]
...
def inference(model_dir,use8bit=False):
    ckpt = '%s/checkpoint-%s'%(model_dir,'45000' if '13b' in model_dir else '70000')
    print(ckpt)
    model = AutoModelForCausalLM.from_pretrained(
        ckpt,
    ...
    o = open(model_dir+'.test.tsv', 'w', encoding="utf-8")
    o2 = open(model_dir+'.time', 'w')
    
    for i, q in data.items():
        if int(i) not in q_tst:
            continue
        print(i)
